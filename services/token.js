...
async function checkToken(token){
  ...
  if (user){
    const token = jwt.sign({_id:__id},'clavesecretaparagenerartoken',{expiresIn:'120'});
...
export  default {
  encode: async (_id, rol, email) =>{
    const token = jwt.sign({_id:_id, rol:rol, email:email}, 'clavesecretaparagenerartoken', {expiresIn: '120'});
    return token;
  },
  decode: async (token) => {
    try {
